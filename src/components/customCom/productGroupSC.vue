<template>
    <div class="topleftBar" id="topleftBar" :class="'topleftBar-'+pageData.pageInteraction">
        ...
        <div class="topbar-box" v-if="pageData.type=='productGroup1'">
            <div class="topbar topbarSC">
                <div class="topbar-item" :class='{activeSC:currentTab==index}' @click="swichNav(index)" v-for="(item,index) in pageData.list" :key="index">
                    <div class="topbar-item-name">{{item.groupName}}</div>
                    <div class="topbar-item-detail">{{item.groupName}}</div>
                    <img :src="pageData.menuBgUrl" v-show="currentTab==index" alt="">
                </div>
            </div>
            <!-- 双列 -->
            <swiper id="topbar-box" v-bind:style="{minHeight:testHeight+'rpx',paddingTop:10+'px'}" :current="currentTab" @change='switchTab' duration="300">
                    ...
                    <scroll-view v-bind:style="{minHeight:testHeight+'rpx'}">
                        <div class="paoductSide">
                            <div @click="goUrl(ele.businessType,ele.merchantProdcutId,ele.productCode)"  class="product-box" v-for="(ele,i) in pageData.list[index].data" :key="i">
                                ...
                                <div class="subhead">
                                    <span class="sales-num">已售{{ele.sellNum}}</span>
                                    <span class="icon-yuan" :style="{color:ele.priceShow?'#fc4150':'transparent'}">￥</span> {{ele.priceShow}} <span style="color: #bcbcbc;">起</span>
                                </div>
                            </div>
                        </div>
                    </scroll-view>
