<template>
  <div class="calendar">
    <div class="calendar-tools">
      <div v-if='!(year==nowYear&&month==nowMonth)' class="calendar-prev" @click="prev">
        <span class="xx-icon icon-xiangzuojiantou jiantou-svg"></span>
      </div>
      <div class="calendar-next" @click="next">
        <span class="xx-icon icon-xiangyoujiantou jiantou-svg"></span>
      </div>
      <div class="calendar-info" @click.stop="changeYear">
        <div class="month">
          <div class="month-inner" :style="{'top': monthPosition +'rpx'}" v-if="isIos">
            <span v-for="m in months" :key="m" class="month-inner-text">{{m}}</span>
          </div>
...
import { arrowLeft, arrowRight } from "../common/calendar/images";
export default {
  props: {
    multi: {
      type: Boolean,
  ...
  methods: {
    ...
    select(k1, k2, e) {
      ...
      if (this.range) {
        ...
        const filterDayIndex = this.multiDays.findIndex(
          v =>
            this.year === v[0] &&
            this.month === v[1] - 1 &&
...
.calendar-tools {
  ...
  line-height: 40px;
  color: #5e7a88;
  box-shadow: 0 5rpx 8rpx 5rpx rgba(242, 242, 242, 0.5);
  margin-bottom: 30rpx;
  border-top: 1px solid rgba(200, 200, 200, 0.1);
}
.calendar-tools ._span {
  cursor: pointer;
}
.jiantou-svg {
  color: #999;
...
.calendar-info > .month .month-inner {
  position: absolute;
  left: 0;
  top: 0;
  height: 480rpx;
...
.calendar-info > .month .month-inner > .month-inner-text {
  display: block;
  font-size: 14px;
  height: 20px;
  width: 100px;
...
.calendar ._td.disabled {
  color: #ccc;
  pointer-events: none !important;
  cursor: default !important;
}
.calendar ._td.disabled ._div {
  color: #ccc;
}
.calendar ._td ._span {
  display: block;
...
.calendar ._td .isGregorianFestival,
.calendar ._td .isTerm,
.calendar ._td .isLunarFestival {
  color: #ea6151;
}
