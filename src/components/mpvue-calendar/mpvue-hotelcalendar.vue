<template>
  <div class="mpvue-calendar" style="background: #f0f2f7;" ref="calendar">
    <table cellpadding="5">
      <div class="mc-hotel-block" v-for="(item, index1) in yearMonthArr" :key="index1">
        <div class="mc-head" v-if="!index1" style="margin: 0px;" :class="['mc-head', {'mc-month-range-mode-head': isMonthRange}]">
        ...
        <div :class="['mc-body', {'mc-range-mode': range, 'week-switch': weekSwitch && !isMonthRange, 'month-range-mode': isMonthRange}]"
          v-for="(days, index) in item.monthRangeDays" :key="index">
          <tr v-for="(day,k1) in days" :key="k1" :class="{'gregorianStyle': !lunar}">
            ...
            >
              ...
              <div v-show="child.selected" class="mc-text mc-price">{{child.multiDaysIndex?'离店':'入住'}}</div>
            </td>
          </tr>
        </div>
      </div>
  ...
  methods: {
    getYearMonthArr() {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        obj.title = obj.nyear + '年' + obj.nmonth + '月';
        obj.monthRangeDays = [this.render(obj.nyear,obj.nmonth-1)]
        ymArr.push(obj);
      }
    ...
    renderOption(year, month, i, playload) {
      ...
      if (this.range) {
        ...
        if (
          ...
          options.multiDaysIndex = this.multiDaysData.length;
    ...
    getLunarInfo(y, m, d) {
      ...
      for(let r = 0; r < this.settingData.length; r++){
        if(this.settingData[r].date == lgDate){
          if(new Date(curDate).getTime() > new Date(this.settingData[r].date).getTime())
