<template>
  <div class="mpvue-calendar" ref="calendar">    
    <div class="calandar-month">
      ...
      >
        <div class="month-text">{{item.nmonthC}}月</div>
...
export default {
  ...
  created() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      obj.nmonthC = SectionToChinese(obj.nmonth)
      ymArr.push(obj);
    }
  ...
  methods: {
    ...
    getLunarInfo(y, m, d) {
      ...
      for(let r = 0; r < this.settingData.length; r++){
        ...
        if(this.settingData[r].date == lgDate && new Date(curDate).getTime() <= new Date(this.settingData[r].date).getTime()){
