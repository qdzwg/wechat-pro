<template>
  <div class="mpvue-calendar" ref="calendar">    
    <div class="calandar-month">
      <div
        v-for="(item, index) in yearMonthArr"
        :key="index"
        class='month-item'
        :class="{cur:iscur==index}"
        @click="iscur=index, rendertest(item.nyear, item.nmonth-1)"        
      >
        <div class="month-text">{{item.nnmonth}}月</div>
        <div class="month-border"></div> 
      </div>
    </div>
    ...
    <table cellpadding="5">
      <div class="mc-head" :class="['mc-head', {'mc-month-range-mode-head': isMonthRange}]">
        <div class="mc-head-box">
          <div v-for="(week, index) in weeks" :style="{'width': width}" :key="index" class="mc-week">{{week}}</div>
        </div>
      </div>
      <div :class="['mc-body', {'mc-range-mode': range, 'week-switch': weekSwitch && !isMonthRange, 'month-range-mode': isMonthRange}]"
        ...
        <tr v-for="(day,k1) in days" :key="k1" :class="{'gregorianStyle': !lunar}">          
          ...
          <!-- <td
            v-for="(child,k2) in day"
            :key="k2"
            :class="[{'selected': child.selected, 'disabled': child.disabled || !child.price, 'mc-range-select-one': rangeBgHide && child.selected, 'lunarStyle': lunar, 'mc-range-row-first': k2 === 0 && child.selected, 'month-last-date': child.lastDay, 'month-first-date': 1 === child.day, 'mc-range-row-last': k2 === 6 && child.selected, 'mc-last-month': child.lastMonth, 'mc-next-month': child.nextMonth}, child.className, child.rangeClassName]"
            @click="select(k1, k2, child, $event)"
            class="mc-day"
            :style="{'width': width,'height': height, 'line-height': lineHeight}"
          > -->
            <span :class="[{'mc-date-red': k2 === (monFirst ? 5 : 0) || k2 === 6}, 'calendar-date']">{{child.day}}</span>
            <div v-show="child.selected" class="mc-text mc-price">已选</div>
          </td>
        </tr>
      </div>
    </table>
...
export default {
  ...
  watch: {
    ...
    tileContent() {
      if (this.isRendeRangeMode()) return;
      this.render(this.year, this.month, "_WATCHRENDER_", "tileContent");//直立的
    },
    almanacs() {
      if (this.isRendeRangeMode()) return;
      this.render(this.year, this.month, "_WATCHRENDER_", "almanacs");//年历
    },
    monthRange() {
      if (this.isRendeRangeMode()) return;
  ...
  created() {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      // obj.nmonth = SectionToChinese(obj.nmonth)
      obj.nnmonth = SectionToChinese(obj.nmonth)
      ymArr.push(obj);
    }
    this.yearMonthArr = ymArr;
  ...
  mounted() {
    ...
    let itemWidth = (wx.getSystemInfoSync().windowWidth / 7 - 5).toFixed(5);
    // console.log('262623', wx.getSystemInfoSync().windowWidth)
    // this.itemStyle = {
    //   width: itemWidth + "px",
    //   height: itemWidth + "px",
    //   lineHeight: itemWidth - 8 + "px"
    // };
    itemWidth = itemWidth > 45 ? 45 : itemWidth; 
    this.width = itemWidth + 'px';
    this.height = itemWidth + 'px';
    this.lineHeight = itemWidth - 8 + "px";
    if (!isBrowser) {
      wx.getSystemInfo({
  ...
  methods: {
    ...
    renderOption(year, month, i, playload) {
      ...
      if (this.range) {
        ...
        if (
          ...
          seletSplit[2] === i//日期是当天则 被选中
        ) {
          Object.assign(
            options,
            { day: i, selected: true },
            this.getLunarInfo(year, monthHuman, i),//获取某一天的所有日历信息
          ...
          Object.assign(
            options,
            { day: i, selected: false },
            this.getLunarInfo(year, monthHuman, i),//某一天的详细信息
            this.getEvents(year, monthHuman, i)
          );
          if (this.begin.length) {
            const beginTime = +new Date(
    ...
    watchRender(type) {
      const weekSwitch = this.weekSwitch;//是否只展示一周
    ...
    render(y, m, renderer, payload) {
      ...
      const isCustomRender = renderer === "CUSTOMRENDER";//customernder  自定义
          ...
          for (let i = 1; i <= 7; i++) {
            LastMonthItems.unshift(
              Object.assign(
                ...
                { disabled: false, lastMonth: true }//上个月 不能选择
              )
            );
            lastMonthDay--;
          }
      ...
      if (
        ...
        for (let i = temp.length; i <= 5; i++) {
          ...
          for (let d = start; d <= start + 6; d++) {
            temp[i].push(
              Object.assign(
                { day: d, disabled: true, nextMonth: true },//下个月  不能选择
                this.getLunarInfo(
      ...
      this.monthRangeDays = [this.days];
      //  console.log("6",this.monthRangeDays);     
      return this.days;
    },
    rendeRange() {
      const range = [];
    ...
    getLunarInfo(y, m, d) {
      let lunarInfo = calendar.solar2lunar(y, m, d);//某一天的所有信息  年月日  农历  星座等
      let yearEve = "";
      if (
        lunarInfo.lMonth === 12 &&
      ...
      let lunarValue = lunarInfo.IDayCn;//农历  初几
      ...
      let lFullMonth = myDate.getMonth()+1;
      let lFullDay = myDate.getDate()
      let curDate = lFullYear + '-' + lFullMonth + '-' + lFullDay;
      //小于当前日期不能被选择
      if(new Date(lunarInfoObj.date).getTime() < new Date(curDate).getTime()){
        lunarInfoObj.disabled = true
      }
      //加入价格库存
      for(let r = 0; r < this.settingData.length; r++){
        let lgyear = lunarInfoObj.date.split('-')[0];
        let lgmonth = lunarInfoObj.date.split('-')[1] < 10 ? '0'+lunarInfoObj.date.split('-')[1] : lunarInfoObj.date.split('-')[1];
        let lgday = lunarInfoObj.date.split('-')[2] < 10 ? '0'+lunarInfoObj.date.split('-')[2] : lunarInfoObj.date.split('-')[2];
        let lgDate = lgyear + '-' + lgmonth + '-' + lgday;
        let myDate = new Date();
        let lFullYear = myDate.getFullYear();
        ...
        let curDate = lFullYear + '-' + lFullMonth + '-' + lFullDay;
       if(this.settingData[r].date == lgDate && new Date(curDate).getTime() <= new Date(this.settingData[r].date).getTime()&&this.settingData[r].salePrice){
          
          lunarInfoObj.price = this.settingData[r].salePrice;    
          // lunarInfoObj.stock = this.settingData[r].stock;
          lunarInfoObj.pxArr = this.settingData[r].pxArr;
        }
      } 
      //结束  
      ...
      return lunarInfoObj;//返回每一天的 日历信息
    },
    getEvents(y, m, d) {
    ...
    select(k1, k2, data, e) {
      ...
      // if (data.disabled||!data.price) return;
