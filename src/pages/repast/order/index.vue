      ...
      <div :style="{'margin-top':(navContentHeight+navBarHeight+'px')}">
        <div class="info-order-block">
          <div class="order-info-list">
            <div class="info-order-title">{{eateryInfo.name}}</div>
            <!-- <div class="order-play-date">
              <div class="order-play-date-item">
                <div class="order-play-date-detail">
                  <span class="order-play-date-label">出游日期</span><span>{{playDate}}</span>
                </div>
          ...
          <div class="order-info-list" v-for="(item, index) in ticketType" :key='index'>
            ...
            <div class="order-play-date">
              <div class="order-play-date-item">
                <div class="order-play-date-detail">
                  <span class="order-play-date-price">{{item.priceSettle}}元</span>
                </div>
...
export default {
  ...
  methods: {
    ...
    getPrice (obj) {
      ...
      if (obj == 0) {
        this.ticketType.forEach((item, index) => {
          this.totalPrice += item.priceSettle * item.num;
          this.sumnum += item.num;
        });
        ...
        switch (obj.applyType) {
          ...
          case "type":
            let duseCprice = 0;
            this.ticketType.forEach((item, index) => {
              duseCprice += item.num * item.priceSettle;
              this.sumnum += item.num;
            });
            this.totalPrice =
          ...
          case "used":
            ...
            for (let i = 0; i < this.ticketType.length; i++) {
              dusetotalPrice +=
                this.ticketType[i].priceSettle * this.ticketType[i].num;
              for (let j = 0; j < obj.productInfos.length; j++) {
                if (
                  this.ticketType[i].productCode ==
      ...
      this.ticketType.forEach((item, index) => {
        modelCodes.push(item.productCode);
    ...
    modifyPlayDate() {
      ...
      let routeId = this.eateryInfo.routeId;
      let routeCode = this.eateryInfo.routeCode;
      this.goUrl('pages/route/detail/mpvueCalendar/main?id='+routeId+'&routeCode='+routeCode+'&routeParams='+routeParams+'&playDate='+this.playDate)
    },
    subForm () {
      this.flag = true;
      if (!this.buyerName) {
      ...
      if (this.idcardType==0) {
        if (this.buyerIdNo != ""&&this.buyerIdNo != null&&this.checkIdcard(this.buyerIdNo)) {
          this.flag = false;
          wx.showToast({
            title: "请输入正确的身份证号码！",
            duration: 2000,
      ...
      if (this.flag) {
        ...
        this.ticketType.forEach((item, index) => {
          modelCodes.push(item.productCode);
          amounts.push(item.num);
          if(item.num&&item.priceSettle)
          totalPrice += item.priceSettle * item.num;
        });
        formData.modelCodes = modelCodes.join(',');
        formData.amounts = amounts.join(',');
        if(this.bonus.useBonus){
    ...
    getCoupons () {
      ...
      this.ticketType.forEach((item, index) => {
        if(item.num&&item.priceSettle)
        cpArr.push(item.productCode + "-" + item.priceSettle * item.num);
      });
  ...
  onLoad () {
    ...
    this.eateryInfo = wx.getStorageSync("eateryInfo");
    //获取缓存票型信息
    let arr = [];
    ...
    //   this.totalPrice += item.priceSettle * Number(item.num);
    //   item.priceSettle = item.priceSettle.toFixed(2);
    // })
    // this.totalPrice = this.totalPrice.toFixed(2)
    wx.showShareMenu({
