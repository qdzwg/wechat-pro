<template>
    <div class="info-order"
        ...
        <navbar title='填写订单信息111'></navbar>
        <div class="info-order-body">
            <div :style="{'margin-top':(navContentHeight+navBarHeight+'px')}">
                <div class="info-order-block">
                    <div class="order-info-list">
                        <div class="info-order-title">演出</div>
                        <div class="order-play-date">
                            <div class="order-play-date-item">
                                <div class="order-play-date-detail">
                                    <span class="order-play-date-label">出游日期</span><span class="order-play-font-size">{{playDate}}</span>
                                </div>
                                <span class="order-play-date-label">场次</span><span class="order-play-font-size">{{playDateTime}}</span>
                            </div>
                        </div>
                    </div>
                    <div class="order-info-list"
                         ...
                         :key="index">
                        <div class="info-order-sub-title">{{item.name}}</div>
                        <div class="order-play-date">
                            <div class="order-play-date-item">
                                <div class="order-play-date-detail">
                                    <span class="order-play-date-price">{{item.salePrice}}元</span>
                                </div>
                                <div class="order-play-date-number">数量*{{item.minSaleStock}}</div>
                            </div>
                        </div>
                    </div>
          ...
          <div v-for="(item,index) in showType"
              :key="index"
              class="hotel-item">
              ...
              <div class="hotel-price">
                    ...
                    <div class="info-item"
                        ...
                        <div class="coupon-select">
                            <span style="display:inline-block;width:100%"
                                  v-if="couponName">{{couponName}}</span>
                            <span v-else
                                  class="have-coupon">您有{{couponList.length}}张优惠券</span>
                ...
                <div class="bottom-box">
                    <div class="left-warper">
                        ...
                        <div class="bottom-price-coupon"
                             v-if="couponCode">优惠费用
            ...
            <!-- 证件类型 -->
            <definedPicker :select='idcardTypeListShow'
        ...
        <um-author ref="um_author" @click="getCoupons"></um-author>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            subFormsubForm() {
                ...
                this.goUrl("pages/order/pay/main");
            },
            openModal() {
                this.canScroll = true;
                this.show = true;
            ...
            //     let priceSettle = item.priceSettle;
            ...
            getPrice(obj) {
                ...
                if (obj == 0) {
                    //不使用优惠券
                    this.showType.forEach((item, index) => {
                        this.totalPrice += (item.priceSettle ? item.priceSettle : item.salePrice) * item.minSaleStock;
                        this.sumnum += item.minSaleStock;
                    });
                    ...
                    switch (obj.applyType) {
                        ...
                        case "type":
                            let duseCprice = 0;
                            this.showType.forEach((item, index) => {
                                duseCprice += (item.priceSettle ? item.priceSettle : item.salePrice) * item.minSaleStock;
                            });
                            this.totalPrice =
                        ...
                        case "used":
                            var dusetotalPrice = 0,
                                ucoupontotalp = 0,
                                duseCouponprice;
                            for (let i = 0; i < this.showType.length; i++) {
                                dusetotalPrice +=
                                    (this.showType[i].priceSettle?this.showType[i].priceSettle:this.showType[i].salePrice) * this.showType[i].minSaleStock;
                                for (let j = 0; j < obj.productInfos.length; j++) {
                                    if (
                                        this.showType[i].modelCode ==
                ...
                //   // if (item.modelCode == code) {
                //   //   priceSettle =
                //   //     priceSettle * item.minSaleStock - val > 0 ? priceSettle * item.minSaleStock - val : 0;
                //   // }
                //   // if (val && code && item.modelCode == code) {
                //   //   this.sumnum += item.minSaleStock;
                //   //   this.totalPrice += priceSettle;
                //   // } else {
                //   //   this.sumnum += item.minSaleStock;
                //   //   this.totalPrice += priceSettle * item.minSaleStock;
                //   // }
                //   this.sumnum += item.minSaleStock;
                //   this.totalPrice += item.salePrice * item.minSaleStock;
                //   console.log(this.totalPrice)
                // });
                // this.totalPrice = this.totalPrice - val > 0 ? this.totalPrice - val : 0;
                // this.totalPrice = this.totalPrice.toFixed(2);
                ...
                let formData = {
                    ...
                    showSession: this.playDateTime,
                    couponCode: this.couponCode
                };
                let arr = [];
                this.showType.forEach((item, index) => {
                    modelCodes.push(item.modelCode);
                    amounts.push(item.minSaleStock);
                    paySum += item.salePrice * item.minSaleStock;
                    let obj = {
                ...
                bonusDeductionJson.totalPrice = paySum;
                ...
                formData.bonusDeductionJson= JSON.stringify(bonusDeductionJson);
            ...
            subForm() {
                        ...
                        if (item.isRealName == "T") {
                            ...
                            for (let i = 0; i < realArr.length; i++) {
                                ...
                                if (!realArr[i].name) {
                                    ...
                                    wx.showToast({
                                        title:
                                            item.nickName + "第" + (i + 1) + "个实名游客姓名不能为空！",
                                        duration: 2000,
                                        icon: "none"
                                    });
                                ...
                                if (!realArr[i].idNo) {
                                    ...
                                    if (this.checkIdCard(realArr[i].idNo)) {
                                        this.flag = false;
                                        wx.showToast({
                                            title: "请输入正确的身份证号！",
                                            duration: 2000,
                ...
                if (this.flag) {
                    ...
                    if(this.bonus.useBonus && this.bonus.bonus){
                    ...
                    // if (wx.getStorageSync("FsyyTimesArr")) {
                    //   formData.FsyyTimes = JSON.stringify(
                    //     wx.getStorageSync("FsyyTimesArr")
                    //   );
                    // }
                    let _url = api.order.saveOrder;
                    postData(_url, formData)
                        .then(
                            result => {
                                ...
                                if (result.status != "200") {
                                    ...
                                    getData("/merchant/api/merchantPayType/getMerchantPayType", {
                                        ...
                                        // wx.setStorageSync("xjsc_orderInfo",orderObj)
                                        // this.subFormsubForm()
            ...
            getCoupons() {
                ...
                let cpArr = [],
                    modelprice = "";
                this.showType.forEach((item, index) => {
                  let num = (item.priceSettle?item.priceSettle:item.salePrice) * item.minSaleStock;
                    cpArr.push(item.modelCode + "-" + num);
                });
                modelprice = cpArr.join(",");
                if(modelprice.length && modelprice.length>0){
                    getData(couponUrl, {
        ...
        onLoad() {
            ...
            console.log(typeof (this.cities), this.productCodes, this.parkIds)
            let _this = this;
            //下单页面拦截授权
            ...
            this.playDate = wx.getStorageSync("playDate");
            this.playDateTime = wx.getStorageSync("playDateTime");
            //获取缓存票型信息
            let arr = [];
            arr = wx.getStorageSync("showType");
            arr.forEach((item, index) => {
                if (arr[index].isRealName == "T") {
                    arr[index].list = [];
            ...
            _this.isface = faceFlag;
            this.showType = arr;
            console.log("看看showtypes的返回", this.showType)
            wx.showShareMenu({
        ...
        onShow() {
            //推荐产品 参数
            this.cities = wx.getStorageSync("wxxcx_citys");
            this.productCodes = wx.getStorageSync('xjsc_merchantProductCode')
            this.parkIds = wx.getStorageSync('xjsc_merchantParkId')
            this.flagShow = false;
            if (wx.getStorageSync("xjsc_2018_linkMan")) {
...
<style lang="scss">
    .info-order-body {
        ...
        background: -moz-linear-gradient(to bottom, #000, #f0f2f7, #f0f2f7);
        /* Firefox 3.6 - 15 */
