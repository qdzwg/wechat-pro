      ...
      <div :style="{'margin-top':(navContentHeight+navBarHeight+'px')}">
        <div class="info-order-block">
          <div class="order-info-list">
            <div class="info-order-title">{{routeInfo.routeName}}</div>
...
export default {
  ...
  methods: {
    ...
    //     let priceSettle = item.salePrice;
    ...
    getPrice (obj) {
      ...
      let formData = {
        ...
        routeParams: "",
        wayType: 6,
        orderType: this.$root.$mp.query.orderType,
        marketingType: 0,
      ...
      formData.modelCodes = modelCodes;
    ...
    modifyPlayDate() {
      ...
      let routeId = this.routeInfo.routeId;
      let routeCode = this.routeInfo.routeCode;
    ...
    subForm () {
      ...
      if (this.buyerIdNo != ""&&this.idcardType==0) {
  ...
  onLoad () {
    ...
    this.routeInfo = wx.getStorageSync("routeInfo");
    //获取缓存票型信息
    let arr = [];
    arr = wx.getStorageSync("routeType");
    
    this.ticketType = arr;
    console.log(arr)
    // this.ticketType.map(item=>{
    //   this.totalPrice += item.salePrice * Number(item.num);
    // })
    // this.totalPrice = this.totalPrice.toFixed(2)
    wx.showShareMenu({
